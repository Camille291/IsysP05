    ...
    if ($row > 0) {
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            if (password_verify($password, $data['password'])) {
                ...
                header('Location: page_connexion.php?login_err=password');
                die();
            }
        } else {
            header('Location: page_connexion.php?login_err=email');
            die();
        }
    } else {
        header('Location: page_connexion.php?login_err=already');
        die();
    }
} else {
