                    ...
                    if (filter_var($email, FILTER_VALIDATE_EMAIL)) { // Si l'email est de la bonne forme
                        if ($password === $password_retype) { // Si les deux mots de passe saisis sont identiques
                            ...
                            if (count($availableCardIds) == 0) {
                                header('Location: inscription_A.php?reg_err=no_card_available');
                                die();
                            }
                            ...
                            header('Location: inscription_A.php?reg_err=success');
                            die();
                        } else {
                            header('Location: inscription_A.php?reg_err=password');
                            die();
                        }
                    } else {
                        header('Location: inscription_A.php?reg_err=email');
                        die();
                    }
                } else {
                    header('Location: inscription_A.php?reg_err=email_length');
                    die();
                }
            } else {
                header('Location: inscription_A.php?reg_err=prenom_length');
                die();
            }
        } else {
            header('Location: inscription_A.php?reg_err=nom_length');
            die();
        }
    } else {
        header('Location: inscription_A.php?reg_err=already');
        die();
    }
} else {
    header('Location: inscription_A.php');
    die();
}
?>
